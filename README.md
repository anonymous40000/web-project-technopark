# OlegOverFlow

Клон Stack Overflow, созданный на Django в учебных целях.

## Возможности (пока реализованы не все)

- Система вопросов и ответов
- Фильтрация вопросов по тегам
- Аутентификация пользователей (вход, регистрация)
- Постраничное отображение
- Система голосования

## Страницы

- `/` - Новые вопросы
- `/hot/` - Популярные вопросы (сортировка по голосам)
- `/questions/tag/<tag_name>/` - Вопросы по тегу
- `/questions/<id>/` - Страница вопроса с ответами
- `/ask/` - Задать новый вопрос
- `/auth/login/` - Страница входа
- `/auth/register/` - Страница регистрации
- `/auth/settings/` - Страница настроек пользователя

## Технологии

- **Бэкенд**: Django 5.2
- **Фронтенд**: HTML, CSS, JavaScript
- **База данных**: PostgreSQL
- **Ссылка на проектирование моделей данных**: https://dbdocs.io/eurser/designing-database-models?table=UserProfile&schema=public&view=table_structure (включать надо с VPN)

## Быстрый старт (Docker)

Требуется установленный Docker и Docker Compose plugin.

1) Клонируйте репозиторий
```bash
git clone https://github.com/anonymous40000/web-project-technopark.git
cd web-project-technopark
```
2) Создайте файл окружения

```bash
cp .env.example .env
# при необходимости измените значения
```
3) Соберите и запустите

```bash
sudo docker-compose up --build
```
После первого запуска: применяются миграции, однократно выполняется заполнение БД командой fill_db (используется переменная FILL_RATIO из .env), поднимается dev-сервер на http://0.0.0.0:8000/.
Однократность заполнения контролируется файлом .bootstrapped в корне проекта. Иначе говоря нужно удалить этот файл, чтобы заполнить дб заново. Но! Важно! Между запусками контейнера база данных сохраняется, поэтому удаляя, .boorstrapped вы просто заполняете базу данных еще раз, а чтобы очистить базу данных полностью, используйте:
```bash
sudo docker-compose down -v
```

## ДЗ 6: Production конфигурация и нагрузочное тестирование

В рамках домашнего задания были созданы тестовые учебные конфигурации для развертывания приложения:

### Компоненты
- **Gunicorn**: WSGI-сервер для запуска Django-приложения
  - Конфигурация: `gunicorn.conf.py`

- **Nginx**: Reverse proxy и статический файловый сервер
  - Конфигурация: `nginx.conf`
  - Обработка статических файлов и media
  - Проксирование запросов к Gunicorn
  - Настройки кеширования и сжатия

### Нагрузочное тестирование
Результаты нагрузочного тестирования с помощью wrk доступны в файле `load_testing.txt`:
- Метрики RPS (requests per second)
- Анализ латентности и пропускной способности

Для просмотра результатов:
```bash
cat load_testing.txt
```
